...
summary: 更换有缺陷的 Cisco Nexus 3232C 存储交换机。这是一个无中断的程序。 
---
...
[role="lead"]
更换有缺陷的 Cisco Nexus 3232C 存储交换机，而不会中断存储网络运行。此无中断过程包括准备更换交换机和恢复存储网络配置。
...
更换 Cisco Nexus 3232C 交换机必须具有下列特性：
...
此过程将第二个 Nexus 3232C 存储交换机 s2 替换为新的 3232C 交换机 ns2。两个节点分别是 cluster1-01 和 cluster1-02。
...
----
====
. 在工作交换机上运行 `show lldp neighbors`命令，以确认您可以看到两个节点和所有磁盘架：
+
`show lldp neighbors`
