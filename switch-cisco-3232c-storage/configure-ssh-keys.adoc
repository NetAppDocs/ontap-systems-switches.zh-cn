...
summary: 确认已在 Cisco Nexus 3232C 存储交换机上启用 SSH 和 SSH 密钥，以支持 CSHM 和日志收集。 
---
= 验证 Cisco Nexus 3232C 存储交换机的 SSH 配置
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
验证是否在 Cisco Nexus 3232C 存储交换机上启用了 SSH 和 SSH 密钥，以支持以太网交换机运行状况监视器 (CSHM) 和日志收集功能。
