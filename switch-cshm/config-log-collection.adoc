...
. 使用在ONTAP端重新生成ssh密钥 `debug system regenerate-systemshell-key-pair`
. 使用重新运行日志收集设置例程 `system switch ethernet log setup-password`
...
* 对于NVIDIA交换机、必须允许日志收集用户在不显示密码提示的情况下运行日志收集命令。要允许使用此命令、请运行以下命令：
