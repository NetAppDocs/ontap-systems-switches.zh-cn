...
summary: 以太网交换机健康监视器 CSHM 日志收集功能用于从 ONTAP 中的集群或存储网络交换机收集日志。 
---
= 配置日志收集
:allow-uri-read: 
:icons: font
...
[role="lead"]
以太网交换机健康监视器（CSHM）负责确保网络交换机的运行健康，并收集交换机日志以进行调试。此过程指导您完成设置收集、请求详细的 *Support* 日志以及启用每小时收集由 AutoSupport 收集的 *Periodic* 数据的过程。
...
CAUTION: 如果日志收集功能报告了任何错误状态（在  `system switch ethernet log show` 的输出中可见），请参阅 link:log-collection-troubleshoot.html["排查日志收集问题"] 以了解详细信息。
