...
summary: '在网络交换机上配置 SNMPv3，以启用与以太网交换机健康监视器 (CSHM) 的安全通信。' 
---
= 配置 SNMPv3 以进行以太网交换机运行状况监控
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在网络交换机上配置 SNMPv3，以启用与以太网交换机运行状况监视器 (CSHM) 的安全、经过身份验证的通信。SNMPv3 为 ONTAP 中的运行状况监控提供加密的交换机状态轮询。

以太网交换机运行状况监视器 (CSHM) 利用 SNMP 来监控网络交换机的运行状况和性能。默认情况下，SNMPv2c 通过参考配置文件 (RCF) 自动配置。SNMPv3 比 SNMPv2 更安全，因为它引入了强大的安全功能，如身份验证、加密和消息完整性，可防止未经授权的访问，并确保传输过程中的数据机密性和完整性。
...
+
** link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-Issues/CONTAP-82891["对于使用ONTAP对Cisco交换机进行健康监控的情况，即使切换到 SNMPv3 进行监控，可能仍然会看到 SNMPv2 流量。"^]
** link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-Issues/CONTAP-119131["当 SNMP 故障发生时，交换机风扇和电源警报可能出现误报。"^]
...
----
. 等待 CSHM 轮询周期结束后，确认以太网交换机的序列号已填充。
+
[source, cli]
----
...
.Cisco 交换机
--
在 Cisco Nexus 交换机上配置 SNMPv3 用户名 SNMPv3_USER：
...
.NVIDIA - CL 5.4.0
--
在运行 CL 5.4.0 的 NVIDIA Cumulus 交换机上配置 SNMPv3 用户名 SNMPv3_USER：
...
.NVIDIA - CL 5.11.0+
--
在运行 CLI 5.11.0 及更高版本的 NVIDIA Cumulus 交换机上配置 SNMPv3 用户名 SNMPv3_USER：
