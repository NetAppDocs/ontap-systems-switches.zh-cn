...
summary: 更换有缺陷的 Cisco Nexus 3232C 集群交换机。这是一个无中断的操作步骤。 
---
...
[role="lead"]
更换有缺陷的 Cisco Nexus 3232C 集群交换机，而不会中断集群操作。此无中断过程包括准备更换交换机和从故障交换机迁移连接。
...
* Nexus 3232C 集群基础架构是冗余的，并且在两台交换机上都能完全正常运行。
+
Cisco以太网交换机页面包含交换机上最新的 RCF 和 NX-OS 版本。

* 所有集群端口必须处于 *up* 状态。
* 两台交换机都必须具备管理连接功能。
* 所有集群逻辑接口 (LIF) 均处于 *up* 状态，且未发生迁移。


更换的 Cisco Nexus 3232C 交换机具有以下特点：
...
* 集群最初有四个节点连接到两个 Nexus 3232C 集群交换机 CL1 和 CL2。
* 您计划将集群交换机 CL2 替换为 C2（步骤 1 至 21）：
+
** 在每个节点上，将连接到集群交换机 CL2 的集群 LIF 迁移到连接到集群交换机 CL1 的集群端口。
** 断开集群交换机 CL2 上所有端口的电缆，并将电缆重新连接到替换集群交换机 C2 上的相同端口。
** 您可以还原每个节点上已迁移的集群 LIF。




.关于示例
此更换过程将第二个 Nexus 3232C 集群交换机 CL2 替换为新的 3232C 交换机 C2。
...
此替换过程中的示例使用四个节点。其中两个节点使用四个 10 GB 集群互连端口：e0a、e0b、e0c 和 e0d。另外两个节点使用两个 40 GB 集群互连端口：e4a 和 e4e。请参阅 link:https://hwu.netapp.com["Hardware Universe"^] 以验证您平台的正确集群端口。
...
. 验证新 Nexus 3232C 交换机上是否安装了适当的 RCF 和映像，并进行任何必要的站点自定义。
+
.. 转到 NetApp 支持站点。
+
http://mysupport.netapp.com/["mysupport.netapp.com"^]

.. 转到 *Cisco 以太网交换机*页面，并在表中记录所需的软件版本。
+
https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco以太网交换机"^]

.. 下载相应版本的 RCF。
.. 单击*描述*页面上的*CONTINUE*，接受许可协议，然后导航到*下载*页面。
...
----
====
. 拆下连接到集群交换机 CL2 的所有电缆，并将其重新连接到所有节点的替换交换机 C2。
. 从集群交换机 CL2 上的端口 e1/31 和 e1/32 上取下交换机间链路 (ISL) 电缆，并将它们重新连接到替换交换机 C2 上的相同端口。
. 在集群交换机 CL1 上打开 ISL 端口 1/31 和 1/32。
+
有关 Cisco 命令的更多信息，请参见 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-command-reference-list.html["Cisco Nexus 3000 系列 NX-OS 命令参考"^] 中列出的指南。
...
[%collapsible]
====
端口 Eth1/31 和 Eth1/32 应指示 (P)，这意味着两个 ISL 端口均已在端口通道中启动。
...
----
====
. 在所有节点上，打开连接到替换交换机 C2 的所有集群互连端口：
+
...
[%collapsible]
====
以下示例显示所有 LIF 已成功恢复，因为在 `Current Port`列下显示的端口在 `Is Home`列中的状态为 `true`。如果端口的值为 `false`，则表示 LIF 尚未恢复。
...
[%collapsible]
====
以下示例显示了集群交换机受到监控，因为 `Is Monitored`状态为 `true`。
