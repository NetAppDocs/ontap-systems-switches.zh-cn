...
summary: '当有更新版本可用时，升级 Cisco Nexus 3232C 集群交换机上的参考配置文件 (RCF)。' 
---
= 升级 Cisco Nexus 3232C 集群交换机上的 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当有较新版本的 RCF 时，升级 Cisco Nexus 3232C 集群交换机上的参考配置文件 (RCF)。
...
CAUTION: 请确保关闭*所有*连接的群集端口，以避免任何网络连接问题。有关详细信息，请参见知识库文章 link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_out_of_quorum_when_migrating_cluster_lif_during_switch_OS_upgrade["在切换操作系统升级期间迁移集群 LIF 时，节点脱离仲裁。"^]。
...
+
有关 Cisco 命令的更多信息，请参见 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 系列 NX-OS 命令参考"^] 中的相应指南。
...
.ONTAP 9.9.1 及更高版本
--
可以使用 `network interface check cluster-connectivity`命令启动群集连接的可访问性检查，然后显示详细信息：
...
.所有ONTAP版本
--
对于所有 ONTAP 版本，您还可以使用
`cluster ping-cluster -node <name>`命令来检查连接：
