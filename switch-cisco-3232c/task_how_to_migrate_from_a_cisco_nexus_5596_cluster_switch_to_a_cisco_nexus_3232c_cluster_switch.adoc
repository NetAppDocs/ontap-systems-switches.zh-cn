...
---
= 如何从 Cisco Nexus 5596 集群交换机迁移到 Cisco Nexus 3232C 集群交换机
:icons: font
:imagesdir: ../media/


[role="lead"]
要将集群中的现有 Cisco Nexus 5596 集群交换机更换为 Nexus 3232C 集群交换机，您必须执行一系列特定的任务。



== 关于此任务

本操作步骤 中的示例介绍了如何将 Cisco Nexus 5596 交换机更换为 Cisco Nexus 3232C 交换机。您可以对其他较早的 Cisco 交换机（例如 3132Q-V ）使用这些步骤（并进行修改）。操作步骤 还使用以下交换机和节点命名：
...
[NOTE]
====
此操作步骤 中的示例使用四个节点：两个节点使用四个 10 GbE 集群互连端口： e0a ， e0b ， e0c 和 e0d 。其他两个节点使用两个 40 GbE 集群互连端口： E4A ， e4e 。_cluster_ Hardware Universe 列出了平台上的实际集群端口。
...
----
. 在所有节点上，启动连接到新 3232C 交换机 C1 的所有集群互连端口： `network port modify`
+
...
----
. 如果未自动删除，请使用 `ssystem cluster-switch delete` 命令删除已更换的 Nexus 5596 ： `ssystem cluster-switch delete -device switch-name`
+
[listing]
----
