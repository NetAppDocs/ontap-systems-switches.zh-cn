...
* 一将集群交换机 CL2 更换为 C2 （步骤 1-22 ）
* 二将集群交换机 CL1 更换为 C1 （步骤 23 至 40 ）


此操作步骤 中的示例使用四个节点：两个节点使用四个 10 GbE 集群互连端口： e0a ， e0b ， e0c 和 e0d 。其他两个节点使用两根 40 GbE 集群互连光缆： E4A 和 e4e 。_cluster_ Hardware Universe 提供了有关平台上的集群光纤缆线的信息。
...
4 paths up, 0 paths down (udp check)
----
. 迁移与第一个 CN1610 交换机 CL1 关联的 LIF ： `network interface migrate -vserver cluster -lif _lif-name_ -source-node _node-name_`
+
...
----
. 验证集群的状态： `network interface show -role cluster`
+
