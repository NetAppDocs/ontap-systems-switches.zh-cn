...
summary: '在 Cisco Nexus 3232C 集群交换机上安装或升级参考配置文件 (RCF)。' 
---
= 在 Cisco Nexus 3232C 集群交换机上安装或升级 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
初始设置后，在 Cisco Nexus 3232C 集群交换机上安装参考配置文件 (RCF)，或在新版本可用时升级 RCF 版本。

您还可以使用此过程升级 RCF 版本。升级 RCF 时，请参阅知识库文章 link:https://kb.netapp.com/onprem/Switches/Cisco/How_to_clear_configuration_on_a_Cisco_interconnect_switch_while_retaining_remote_connectivity["如何在保持远程连接的情况下清除Cisco互连交换机上的配置"^] 以获取更多信息。
...
* link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco以太网交换机页面"^] 有关支持的 ONTAP 和 RCF 版本，请参阅交换机兼容性表。请注意，RCF 中的命令语法与 NX-OS 版本中的命令语法之间可能存在命令依赖关系。
* link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000系列交换机"^]。有关 Cisco 交换机升级和降级过程的完整文档，请参见 Cisco 网站上提供的相应软件和升级指南。
...
* 这两台 Cisco 交换机的名称为 `cs1`和 `cs2`。
* 节点名称为 `cluster1-01`、 `cluster1-02`、 `cluster1-03`和 `cluster1-04`。
...
----
====
. 验证集群端口是否已迁移到集群交换机 cs1 上托管的端口。这可能需要几秒钟才能完成。
+
`network interface show -role cluster`
...
. 记录当前 `running-config`与正在使用的 RCF 文件之间的任何自定义添加内容。
. 保存基本配置详细信息 `write_erase.cfg`启动闪存上的文件。
+

NOTE: 升级或应用新的 RCF 时，必须擦除交换机设置并执行基本配置。您必须连接到交换机串行控制台端口才能重新设置交换机。
...
. 使用以下传输协议之一将 RCF 复制到交换机 cs2 的 bootflash：FTP、TFTP、SFTP 或 SCP。有关 Cisco 命令的更多信息，请参见 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 系列 NX-OS 命令参考"^]
+
.显示示例
[%collapsible]
====
...
NOTE: 首次应用 RCF 时，*ERROR: Failed to write VSH commands* 消息是预期出现的，可以忽略。
...
. 验证 RCF 版本和交换机设置是否正确后，将运行配置文件复制到启动配置文件。
+
有关 Cisco 命令的更多信息，请参见 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 系列 NX-OS 命令参考"^]
...
----
. 重新启动交换机 cs2。可以在交换机重新启动时忽略节点上报告的"集群端口关闭"事件。
+
[listing, subs="+quotes"]
----
cs2# *reload*
...
----
====
.. 验证来自集群的交换机运行状况（这可能不会显示交换机 cs2，因为 LIF 不驻留在 e0d 上）。
+
.显示示例
[%collapsible]
====
...
[NOTE]
====
您可能会在 cs1 交换机控制台上观察到以下输出，具体取决于以前加载在交换机上的 RCF 版本
