...
summary: 首次设置好 Nexus 3232C 交换机后，即可安装 RCF。您也可以使用此方法升级 RCF 版本。 
---
= 安装或升级参考配置文件 (RCF)
:allow-uri-read: 
:icons: font
...
[role="lead"]
首次设置 Nexus 3232C 交换机后，请按照以下步骤安装 RCF。
...
* link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco以太网交换机页面"^]请查阅交换机兼容性列表，了解支持的ONTAP和 RCF 版本。请注意，RCF 中的命令语法与 NX-OS 版本中的命令语法之间可能存在命令依赖关系。
...
. 记录当前版本与当前版本之间的任何自定义添加内容。 `running-config`以及正在使用的 RCF 文件。
...
. 使用以下传输协议之一将 RCF 复制到交换机 cs2 的 bootflash：FTP、TFTP、SFTP 或 SCP。有关 Cisco 命令的详细信息，请参阅 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 系列 NX-OS 命令参考"^] 中的相应指南。
+
.显示示例
[%collapsible]
====
