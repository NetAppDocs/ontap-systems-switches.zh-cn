...
[role="lead"]
您可以在首次设置 Nexus 3232C 交换机后安装 RCF 。您也可以使用此操作步骤 升级 RCF 版本。
...
[NOTE]
====
操作步骤 需要 ONTAP 使用 -se 命令和 Cisco Nexus 3000 系列交换机命令；除非另有说明，否则会使用 ONTAP 命令。
...
----
. 验证集群端口是否已迁移到集群交换机 CS1 上托管的端口。这可能需要几秒钟的时间。`network interface show -role cluster`
+
[listing]
----
cluster1::*> network interface show -role cluster
