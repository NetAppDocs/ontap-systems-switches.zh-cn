...
---
= 如何使用 Cisco Nexus 9336C-x2 集群交换机迁移到交换式 NetApp 集群环境
:icons: font
:imagesdir: ../media/
...
----
.. 验证 ISL 和两个 9336C-x2 交换机 CS1 和 CS2 之间的 ISL 上的物理端口是否已在端口 1/35 和 1/36 上启动： `s如何显示端口通道摘要`
+
...
----
.. 确认 `Is Home` ： `network interface show -vserver Cluster` 的所有集群 LIF 均已启动，正常运行并显示为 true
+
...
4 entries were displayed.
----
.. 确认 `的所有接口均显示 true ，并且为 Home` ： `network interface show -vserver Cluster`
+
...
Total entries displayed: 4
----
.. 显示有关集群中发现的网络设备的信息： `network device-discovery show -protocol cdp`
+
[listing]
----
