...
summary: 从旧的 Cisco 集群交换机迁移到 NVIDIA SN2100 集群交换机，而不会中断集群操作。 
---
= 从 Cisco 集群交换机迁移到 NVIDIA SN2100 集群交换机
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在不中断集群操作的情况下，将集群交换机从 Cisco 迁移到 NVIDIA SN2100 集群交换机。在开始迁移之前，请查看配置信息、端口连接和布线要求。
...
* 有关支持的端口及其配置的完整详细信息，请参见 link:https://hwu.netapp.com/["Hardware Universe"^]。
* 您已将NVIDIA SN2100 交换机上的一些端口配置为以 40 GbE 或 100 GbE 运行。
* 您已规划、迁移并记录了从节点到NVIDIA SN2100 集群交换机的 40 GbE 和 100 GbE 连接。
+

NOTE: 在此过程中，使用Cisco Nexus 3232C 集群交换机作为示例命令和输出。
...
. 将权限级别更改为高级，并在提示继续时输入 *y*： `set -privilege advanced`
+
出现高级提示符（*>）。

. 禁用集群 LIF 的自动回滚功能： `network interface modify -vserver Cluster -lif * -auto-revert false`
+
[listing, subs="+quotes"]
----
...
e3b       Cluster    Cluster          up   9000  auto/100000  healthy  false
----
.. 显示有关逻辑接口及其指定归属节点的信息： `network interface show -vserver Cluster`
+
...
. 从节点的角度来看，每个节点上的集群端口通过以下命令以如下方式连接到现有的集群交换机： `network device-discovery show -protocol lldp`
+
[listing, subs="+quotes"]
----
...
----
. 集群端口和交换机通过以下命令连接（从交换机的角度来看）： `show cdp neighbors`
+
[listing, subs="+quotes"]
----
...
--
====
. [[step8]] 在交换机 c2 上，关闭与节点集群端口连接的端口。
+
[listing, subs="+quotes"]
----
(c2)# *configure*
...
----
. 在交换机 sw2 上，确认所有节点集群端口均已启动： `net show interface`
+
[listing, subs="+quotes"]
----
...
node2      /lldp
            ...
            e3b    sw2 (b8:ce:f6:19:1b:96)   swp4            -
----
. 在交换机 sw1 和 sw2 上，验证所有节点集群端口是否都已启动： `net show interface`
+
[listing, subs="+quotes"]
----
...
UP     swp16        100G  9216   BondMember  sw1 (swp16)       Master: cluster_isl(UP)
----
. 确认两个节点都分别与每个交换机建立了一条连接： `net show lldp`
+
以下示例显示了两种开关的正确结果：
...
----
. 启用集群 LIF 的自动回滚功能： `cluster1::*> network interface modify -vserver Cluster -lif * -auto-revert true`
. 确认所有集群网络 LIF 都已恢复到其原端口： `network interface show`
+
[listing, subs="+quotes"]
----
cluster1::*> *network interface show -vserver Cluster*
...
Cluster
            ...
            node2_clus2  up/up    169.254.19.183/16  node2         e3b     true
----
. 启用以太网交换机健康监控日志收集功能，以收集与交换机相关的日志文件，使用以下两个命令： `system switch ethernet log setup-password`和 `system switch ethernet log enable-collection`
+
...
. 启动交换机日志收集功能： `system switch ethernet log collect -device *`
+
...
----
. 将权限级别改回管理员： `set -privilege admin`
. 如果您已禁用自动创建案例功能，请通过调用AutoSupport消息重新启用该功能： `system node autosupport invoke -node * -type all -message MAINT=END`
