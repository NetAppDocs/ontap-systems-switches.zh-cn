...
summary: 更换有缺陷的 NVIDIA SN2100 存储交换机。这是一个无中断的程序。 
---
= 更换 NVIDIA SN2100 存储交换机
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更换有缺陷的 NVIDIA SN2100 存储交换机，而不会中断存储网络运行。此无中断过程包括准备更换交换机和恢复存储网络配置。

.开始之前
在 NVIDIA SN2100 存储交换机上安装 Cumulus 软件和 RCF 之前，请确保：

* 您的系统可以支持NVIDIA SN2100 存储交换机。
* 您已下载适用的 RCF 文件。


link:http://hwu.netapp.com["Hardware Universe"^] 提供了受支持端口及其配置的完整详细信息。
