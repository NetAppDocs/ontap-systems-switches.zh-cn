...
summary: 在 Cisco Nexus 9332D-GX2B 交换机上安装 NX-OS，用于新的 AFX 1K 或 AFX 2K 系统安装、交换机更换或出厂默认重置。 
---
= 在 Cisco Nexus 9332D-GX2B 交换机上安装 NX-OS
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 Cisco Nexus 9332D-GX2B 交换机上安装 NX-OS，适用于新的 AFX 1K 或 AFX 2K 系统安装、交换机更换或将交换机重置为出厂默认设置。
...
在第一台交换机（cs1）上安装并验证 NX-OS 和 EPLD 映像，然后如果您要将 NX-OS 作为新 AFX 系统安装的一部分进行安装，请在第二台交换机（cs2）上重复这些步骤。
...
. [[stepa]]将 NX-OS 和 EPLD 映像复制到第一个交换机。
+
...
+
以下示例显示了从 NX-OS 10.4(2)F 到 10.6(3)F 的升级。第一次执行 NX-OS 升级，第二次执行 EPLD 映像升级。
...
....
=====
. 交换机重新启动后，请重新登录并验证新版本的 EPLD 是否已成功加载：
+
`show version module 1 epld`
