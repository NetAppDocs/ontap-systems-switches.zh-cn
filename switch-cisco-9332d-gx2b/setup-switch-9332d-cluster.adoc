...
====
* 适用的许可证、网络和配置信息以及缆线。
* 适用的NetApp共享网络和管理网络 RCF 可从NetApp支持站点下载，网址为 http://mysupport.netapp.com/["mysupport.netapp.com"^]。所有Cisco共享网络和管理网络交换机均采用标准Cisco出厂默认配置。这些交换机也具有当前版本的 NX-OS 软件，但未加载 RCF。
* link:required-documentation-9332d-cluster.html["所需的交换机和ONTAP 文档"] 。
...
. 检查共享网络交换机上的版本，如有必要，NetApp https://software.cisco.com/download/home["Cisco软件下载"^]页。


.下一步是什么？
配置好交换机后，link:install-nxos-overview-9332d-cluster.html["准备安装NX-OS软件和RCF"] 。
