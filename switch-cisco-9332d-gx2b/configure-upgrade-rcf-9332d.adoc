...
summary: '您可以升级 Cisco Nexus 9332D-GX2B 交换机上的参考配置文件 (RCF) 版本，以支持新硬件、新 ONTAP 版本或与 AFX 1K 或 AFX 2K 系统配置中的新 NX-OS 版本保持一致。' 
---
= 升级 RCF 版本 - Cisco Nexus 9332D-GX2B
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
您可以升级 Cisco Nexus 9332D-GX2B 交换机上的参考配置文件 (RCF) 版本，以支持新硬件、新 ONTAP 版本或与 AFX 1K 或 AFX 2K 系统配置中的新 NX-OS 版本保持一致。
...
CAUTION: 请确保关闭*所有*连接的端口，以避免任何网络连接问题。有关详细信息，请参见知识库文章 https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_out_of_quorum_when_migrating_cluster_lif_during_switch_OS_upgrade["在切换操作系统升级期间迁移集群 LIF 时，节点脱离仲裁。"^]。
...
+
如果您的交换机运行的是 RCF 10.1 或更高版本，则可以在需要时 link:configure-install-upgrade-rcf-overview-9332d.html#change-port-speed-configuration["更改端口速度配置"]。
...
.下一步是什么？
升级交换机上的 RCF 后，您link:configure-verify-ssh-9332d.html["验证日志收集的 SSH"]。
