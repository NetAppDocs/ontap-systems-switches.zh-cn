...
summary: '当有较新版本可用时，升级 Cisco Nexus 9336C-FX2 共享交换机上的参考配置文件 (RCF)。' 
---
= 在 Cisco Nexus 9336C-FX2 共享交换机上升级 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当有较新版本的 RCF 时，升级 Cisco Nexus 9336C-FX2 共享交换机上的参考配置文件 (RCF)。
...
----
====
. 检查每个集群端口、节点存储端口和存储盘架端口的管理和运行状态。
+
.. 确认集群所有端口均已启动且状态正常：
+
...
+
.. 记录当前运行配置和正在使用的 RCF 文件之间的任何自定义添加内容（例如，贵组织的 SNMP 配置）。
...
. 有关需要进行的任何进一步更改的详细信息，请参阅 link:cabling-considerations-9336c-shared.html["审查布线和配置注意事项"]。
...
====
.下一步是什么？
升级 RCF 后，您可以 link:../switch-cshm/config-overview.html["配置交换机健康监控"]。
