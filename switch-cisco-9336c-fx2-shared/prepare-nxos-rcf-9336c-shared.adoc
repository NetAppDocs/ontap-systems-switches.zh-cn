...
keywords:  
summary: '在安装 NX-OS 软件和参考配置文件 (RCF) 之前，请准备好 Cisco Nexus 9336C-FX2 共享交换机。' 
---
= 准备在 Cisco Nexus 9336C-FX2 共享交换机上安装 NX-OS 软件和 RCF
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
在安装 NX-OS 软件和参考配置文件 (RCF) 之前，请准备好 Cisco Nexus 9336C-FX2 共享交换机。必须先安装 NX-OS 和 RCF，然后交换机才能在 ONTAP 环境中运行。
...
----
====
. 检查每个集群接口以及每个节点存储端口和存储盘架端口的管理或运行状态。
+
.. 显示网络端口属性：
+
[source, cli]
...
.下一步是什么？
准备好安装 NX-OS 软件和 RCF 后，您可以…… link:install-nxos-software-9336c-shared.html["安装 NX-OS 软件"]。
