...
summary: '初始设置后，在 Cisco Nexus 9336C-FX2 共享交换机上安装参考配置文件 (RCF)，以配置组合集群和存储操作的端口分配。' 
---
= 在Cisco Nexus 9336C-FX2 共享交换机上安装 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
初始设置后，在 Cisco Nexus 9336C-FX2 共享交换机上安装参考配置文件 (RCF)。RCF 为组合集群和存储操作配置端口分配。

开始之前，请先完成以下步骤。link:prepare-nxos-rcf-9336c-shared.html["准备安装 NX-OS 和 RCF"] 。
...
.可用的 RCF 配置
* *ClusterStorageRCF* - 支持交换机上的分区集群和两个存储区域（Cluster-Storage RCF 1._xx_）。
...
.下一步是什么？
安装完 RCF 后，您可以…… link:../switch-cshm/config-overview.html["配置交换机健康监控"]。
