...
* 您必须具有所需的共享交换机文档。
+
请参见  documentation for shared switches 有关详细信息 ...
...
. 【第 3 步】根据中提供的信息对交换机执行初始配置  configuration information。
. 验证您在设置结束时显示的屏幕中所做的配置选择，并确保保存此配置。
. 检查交换机上的软件版本，如有必要，请将 NetApp 支持的软件版本下载到这些交换机。
+
如果您下载的是 NetApp 支持的软件版本，则还必须下载 NetApp 网络交换机参考配置文件并将其与保存在中的配置合并 ,第 3 步。您可以从下载文件和说明 https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco 以太网交换机"] 页面。如果您有自己的交换机，请参见 http://www.cisco.com["Cisco"] 站点
