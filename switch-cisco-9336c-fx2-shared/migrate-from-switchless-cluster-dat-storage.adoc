...
keywords:  
summary: 通过添加两台新的共享交换机，从具有直连存储的无交换机集群迁移到 Cisco Nexus 9336C-FX2 共享交换机。 
---
= 从具有直连存储的无交换机集群迁移到 Cisco Nexus 9336C-FX2 共享交换机
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
通过添加两台新的共享交换机，从具有直连存储的无交换机集群迁移到 Cisco Nexus 9336C-FX2 共享交换机。

具体操作步骤取决于每个控制器上是有两个专用集群网络端口还是只有一个集群端口。记录的过程适用于所有使用光纤或 Twinax 端口的节点，但如果节点使用板载 10Gb BASE-T RJ45 端口作为集群网络端口，则此交换机不支持此过程。

大多数系统在每个控制器上需要两个专用集群网络端口。有关详细信息，请参见 link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco以太网交换机"^]。
...
* cluster1::*> 提示符指示集群的名称。
* 根据 AFF A400 控制器，此过程中使用的集群端口为 _e3a_ 和 _e3b_。link:https://hwu.netapp.com["Hardware Universe"^] 包含有关平台的实际集群端口的最新信息。
...
+
link:https://hwu.netapp.com["Hardware Universe"^] 包含有关布线的详细信息。有关交换机安装要求的详细信息，请参见 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["安装HWU中没有的设备还需要哪些额外信息？"^]。
