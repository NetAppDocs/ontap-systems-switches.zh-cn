...
* 您必须参考上的交换机兼容性表 http://["NetApp CN1601 和 CN1610 交换机"^] 页面上显示了受支持的 FastPath ， RCF 和 ONTAP 版本。
+
...
NOTE: 在 RCF 1.2 版中，出于安全考虑，已明确禁用对 Telnet 的支持。要在安装 RCF 1.2 时避免连接问题，您必须验证是否已启用安全外壳（ SSH ）。。 https://["《 NetApp ® CN1610 交换机管理员指南》"^] 了解有关 SSH 的详细信息。
