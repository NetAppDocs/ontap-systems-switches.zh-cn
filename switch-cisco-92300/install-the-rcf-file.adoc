...
summary: '在初始设置后或有更新版本可用时，在 Cisco Nexus 92300YC 集群交换机上安装或升级参考配置文件 (RCF)。' 
---
= 在 Cisco Nexus 92300YC 集群交换机上安装或升级 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
初始设置后，在 Cisco Nexus 92300YC 集群交换机上安装参考配置文件 (RCF)，或在新版本可用时升级您的 RCF 版本。
...
[NOTE]
====
* 该过程需要同时使用 ONTAP 命令和 https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["CiscoNexus 9000 系列交换机"^]；除非另有说明，否则使用 ONTAP 命令。
* 在执行此过程之前，请确保您有交换机配置的当前备份。
...
CAUTION: 更新或应用新的 RCF 时，必须擦除交换机设置并执行基本配置。您必须连接到交换机串行控制台端口才能重新设置交换机。
...
. 使用以下传输协议之一将 RCF 复制到交换机 cs2 的 bootflash：FTP、TFTP、SFTP 或 SCP。有关 Cisco 命令的更多信息，请参见 link:https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["CiscoNexus 9000 系列交换机"^] 中的相应指南。
+
...
+
有关 Cisco 命令的更多信息，请参见 link:https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["CiscoNexus 9000 系列交换机"^] 中的相应指南。

+
此示例显示了在交换机 cs2 上安装的 RCF 文件 `Nexus_92300YC_RCF_v1.0.2.txt`：
...
. 重新应用之前对交换机配置所做的所有自定义设置。有关所需的任何进一步更改的详细信息，请参阅 link:cabling-considerations-92300.html["审查布线和配置注意事项"]。
...
----
. 验证集群 LIF 是否已迁移到交换机 cs2 上托管的端口。这可能需要几秒钟才能完成。 `network interface show -vserver Cluster`
+
.显示示例
[%collapsible]
====
...
----
. 重新启动交换机 cs1。执行此操作可触发集群 LIF 恢复到其主端口。您可以在交换机重新启动时忽略节点上报告的"集群端口关闭"事件。
+
[listing, subs="+quotes"]
----
cs1# *reload*
...
====
.下一步是什么？
安装 RCF 后，您可以link:configure-ssh-keys.html["验证 SSH 配置"]。
