...
summary: '初始设置后，在 Cisco Nexus 92300YC 集群交换机上安装参考配置文件 (RCF)。' 
---
= 在 Cisco Nexus 92300YC 集群交换机上安装 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
初始设置后，在 Cisco Nexus 92300YC 集群交换机上安装参考配置文件 (RCF)。RCF 定义了集群操作所需的端口分配和网络设置。
...
* 节点名称是 `node1`和 `node2`。
* node1 的集群 LIF 名称为 `node1_clus1`和 `node1_clus2`，node2 的集群 LIF 名称为 `node2_clus1`和 `node2_clus2`。
* 这 `cluster1::*>`prompt 指示集群名称。
* 每个节点上的集群端口都命名为 `e0a`和 `e0b`。
+
有关平台上支持的实际集群端口，请参见 link:https://hwu.netapp.com/Home/Index["Hardware Universe"^]。有关交换机安装要求的详细信息，请参见 link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["安装HWU中没有的设备还需要哪些额外信息？"^]。



.支持的端口和节点连接
* Nexus 92300YC 交换机支持的交换机间链路 (ISL) 是端口 1/65 和 1/66。
* Nexus 92300YC 交换机支持的节点连接为端口 1/1 到 1/66。
* 此过程中的示例使用两个节点。这些节点使用两个 10GbE 集群互连端口 `e0a`和 `e0b`。请参见 link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe"^] 以验证平台上的正确群集端口。


.开始之前
* 您必须具有当前的RCF。
* 集群必须完全运行（日志中应无错误或类似问题）。
* 如果仅安装 NX-OS 并保留当前的 RCF 版本，则必须在 RCF 中检查或设置所需的引导配置，以反映所需的引导映像。
* 如果需要更改启动配置以反映当前的启动映像，则必须在重新应用 RCF 之前进行更改，以便在以后的重启中实例化正确的版本。
* 对于支持的 ONTAP、NX-OS 和 RCF 版本，您必须已查阅 https://support.netapp.com/NOW/download/software/cm_switches/["Cisco 以太网交换机"^]页面上的交换机兼容性表。
...
Copy complete.
----
. 对于 ONTAP 9.6P8 及更高版本，使用以下命令启用以太网交换机健康监视器日志收集功能，以收集与交换机相关的日志文件： `system cluster-switch log setup-password` 和 `system cluster-switch log enable-collection`
+
[listing, subs="+quotes"]
----
