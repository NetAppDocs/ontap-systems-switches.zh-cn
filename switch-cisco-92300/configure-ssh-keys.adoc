...
summary: 确认已在 Cisco Nexus 92300YC 集群交换机上启用 SSH 和 SSH 密钥，以支持 CSHM 和日志收集。 
---
= 验证 Cisco Nexus 92300YC 集群交换机的 SSH 配置
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
验证是否在 Cisco Nexus 92300YC 集群交换机上启用了 SSH 和 SSH 密钥，以支持以太网交换机运行状况监控器 (CSHM) 和日志收集功能。
...
IMPORTANT: 启用 FIPS 时，必须使用命令 `ssh key ecdsa 256 force`将交换机上的位数更改为 256。有关更多详细信息，请参见 https://docs.netapp.com/us-en/ontap/networking/configure_network_security_using_federal_information_processing_standards_@fips@.html#enable-fips["使用 FIPS 配置网络安全"^]。
