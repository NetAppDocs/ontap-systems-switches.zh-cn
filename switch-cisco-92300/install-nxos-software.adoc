...
summary: 在 Cisco Nexus 92300YC 集群交换机上安装 NX-OS 软件，为集群运行做好准备。 
---
= 在 Cisco Nexus 92300YC 集群交换机上安装 NX-OS 软件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 Cisco Nexus 92300YC 集群交换机上安装 NX-OS 软件，为集群运行做好准备。必须先安装 NX-OS，然后才能配置交换机或安装 RCF。

NX-OS 是 Cisco Systems 为 Nexus 系列以太网交换机和 MDS 系列光纤通道 (FC) 存储区域网络交换机提供的网络操作系统。
...
* 从 NetApp 支持站点为您的交换机获取适用的 NetApp Cisco NX-OS 软件，可从 http://mysupport.netapp.com/["mysupport.netapp.com"]
...
此过程中的示例使用两个节点，但集群中最多可以有 24 个节点。

.关于示例
本流程中的示例使用以下开关和节点命名规则：

* Nexus 92300YC 交换机名称为 `cs1`和 `cs2`。
* 本过程中使用的示例在第二个交换机 _*cs2*_ 上启动升级。
...
.下一步是什么？
安装 NX-OS 软件后，您可以link:install-the-rcf-file.html["安装 RCF"]。
