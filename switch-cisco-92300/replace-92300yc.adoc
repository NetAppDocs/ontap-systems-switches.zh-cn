...
summary: 更换集群网络中存在故障的 Cisco Nexus 92300YC 集群交换机。这是一个无中断的操作过程。 
---
= 更换 Cisco Nexus 92300YC 集群交换机
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在不中断集群运行的情况下，更换集群网络中有缺陷的 Cisco Nexus 92300YC 集群交换机。此无中断程序（NDU）涉及准备替换交换机以及从故障交换机迁移连接。
...
* 对于 Nexus 92300YC 替换交换机：
+
** 替换交换机的管理网络连接功能正常。
** 控制台已就绪，可访问替换开关。
** 节点连接是端口 1/1 到 1/64。
** 所有交换机间链路 (ISL) 端口在端口 1/65 和 1/66 上均已禁用。
** 将所需的参考配置文件 (RCF) 和 NX-OS 操作系统映像交换机加载到该交换机上。
** 该交换机的初始定制已完成，详见：link:configure-install-initial.html["配置 Cisco Nexus 92300YC 交换机"]。
+
...
* 现有 Nexus 92300YC 交换机的名称为 cs1 和 cs2。
...
.. 单击浏览器的后退箭头返回 *Description* 页面，单击 *CONTINUE*，接受许可协议，然后转到 *Download* 页面。
...
----
====
. 卸下 Nexus 92300YC cs2 交换机上的所有电缆，然后将其连接到 Nexus 92300YC newcs2 交换机上的相同端口。
. 打开 cs1 和 newcs2 交换机之间的 ISL 端口 1/65 和 1/66，然后验证端口通道运行状态。
+
端口通道应表示 Po1(SU)，成员端口应表示 Eth1/65(P) 和 Eth1/66(P)。

+
.显示示例
[%collapsible]
====
此示例启用了 ISL 端口 1/65 和 1/66，并显示了交换机 cs1 上的端口通道摘要：
