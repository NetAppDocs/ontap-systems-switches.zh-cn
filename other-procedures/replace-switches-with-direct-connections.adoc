...
summary: 您可以将双节点集群中的交换机更换为节点之间的直接背对背连接。 
---
= 将交换机更换为直接连接
:allow-uri-read: 
:icons: font
...
如果您的双节点集群使用集群互连交换机、请将交换机更换为节点之间的直接背对背连接。这是一项无中断操作。

.开始之前
* 集群必须运行状况良好、并且包含两个节点、这些节点由集群交换机连接、并且必须运行相同的ONTAP 版本。
* 每个节点都必须具有所需数量的专用集群端口、这些端口可提供冗余集群互连连接以支持您的系统配置、例如、对于每个节点上具有两个专用集群互连端口的系统、需要两个冗余端口。
...
+
验证端口的"Link"列值是否为`up`、而"Health Status"列值是否为`healthy`、如以下示例所示：
...
+
"Discovered Device"列应是端口所连接的集群交换机的名称。以下示例显示集群端口"e0a"和"e0b"已正确连接到集群交换机"CS1"和"CS2"。
...
+
在以下示例中、node1上的"e0a"连接到node2上的"e0a"、node2上的"e0b"连接到node2上的"e0b"：
...
link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/How_to_suppress_automatic_case_creation_during_scheduled_maintenance_windows_-_ONTAP_9["NetApp 知识库文章 1010449 ： How to suppress automatic case creation during scheduled maintenance windows."^]
