...
summary: '当有更新版本可用时，升级 Cisco Nexus 3132Q-V 集群交换机上的参考配置文件 (RCF)。' 
---
= 升级 Cisco Nexus 3132Q-V 集群交换机上的 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当有较新版本的 RCF 时，升级 Cisco Nexus 3132Q-V 集群交换机上的参考配置文件 (RCF)。
...
NOTE: 对于 ONTAP 9.8 及更高版本，请使用命令 `system switch ethernet show -is-monitoring-enabled-operational true`。
...
NOTE: 显示的端口数量根据集群中的节点数量而有所不同。

. 验证集群端口是否已故障切换到集群交换机 cs1 上托管的端口。这可能需要几秒钟才能完成。
+
`network interface show -vserver Cluster`
...
. 记录当前运行配置和正在使用的 RCF 文件之间的任何自定义添加内容。
+
[NOTE]
====
确保配置以下内容：
...
NOTE: 在升级或应用新的 RCF 时，您必须清除交换机设置并执行基本配置。
...
+
有关 Cisco 命令的更多信息，请参见 https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 系列 NX-OS 命令参考"^]中的相应指南。
...
NOTE: 有关如何在 RCF 升级后使 10GbE 端口联机的步骤，请参见知识库文章 link:https://kb.netapp.com/onprem%2FSwitches%2FCisco%2F10GbE_ports_on_Cisco_3132Q_cluster_switch_do_not_come_online["Cisco 3132Q 集群交换机上的 10GbE 端口未联机"^]。



. 验证 RCF 版本和交换机设置是否正确后，将 `running-config`文件复制到 `startup-config`文件中。
+
有关 Cisco 命令的更多信息，请参见 link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 系列 NX-OS 命令参考"^] 中的相应指南。Show 示例
...
----
====
. 重启交换机 cs2。交换机重启时，您可以忽略节点上报告的"集群端口关闭"事件以及错误 `% Invalid command at '^' marker`输出。
+
[listing, subs="+quotes"]
----
cs2# *reload*
This command will reboot the system. (y/n)?  [n] *y*
----
. 重新应用之前对交换机配置所做的所有自定义设置。有关所需的任何进一步更改的详细信息，请参阅 link:cabling-considerations-3132q-v.html["审查布线和配置注意事项"]。
. 检查集群上集群端口的运行状况。
+
