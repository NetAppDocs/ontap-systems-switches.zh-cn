...
summary: '初始设置后，在 Cisco Nexus 3132Q-V 集群交换机上安装参考配置文件 (RCF)。' 
---
= 在 Cisco Nexus 3132Q-V 集群交换机上安装 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
初始设置后，在 Cisco Nexus 3132Q-V 集群交换机上安装参考配置文件 (RCF)。RCF 定义了集群操作所需的端口分配和网络设置。
...
+
有关更多详细信息，请参阅 link:https://kb.netapp.com/on-prem/Switches/Cisco-KBs/How_to_clear_configuration_on_a_Cisco_interconnect_switch_while_retaining_remote_connectivity["如何在保持远程连接的情况下清除Cisco互连交换机上的配置"^]。
...
有关设置说明，请参见 link:https://docs.netapp.com/us-en/ontap/task_configure_ontap.html["使用 System Manager 在新集群上配置ONTAP"]。
