...
* 现有集群网络配置必须具有：
+
** Nexus 3132 集群基础架构是冗余的，可在两台交换机上完全正常运行。
+
...
[NOTE]
====
此操作步骤 中的示例使用两个节点，每个节点使用两个 40/100 GbE 集群互连端口 E4A 和 e4e 。_cluster_ Hardware Universe 提供了有关平台上集群端口的详细信息。
...
* 第一个集群端口已移至 C1 （步骤 1 至 20 ）。
* 第二个集群端口已移至 C2 （步骤 21 至 32 ）。
* 禁用双节点无交换机集群选项（步骤 33 至 35 ）。
...
[NOTE]
====
将任何缆线重新连接到新的 Cisco 集群交换机时，使用的缆线必须为光纤或 Cisco 支持的缆线。
...
----
. 验证是否已禁用 `sswitchless-cluster` 选项。
+
[listing]
----
