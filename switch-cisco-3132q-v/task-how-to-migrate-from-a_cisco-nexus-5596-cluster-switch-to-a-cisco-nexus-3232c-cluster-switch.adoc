...
---
= 如何将 Cisco Nexus 5596 集群交换机更换为 Cisco Nexus 3132Q-V 集群交换机
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.关于此任务
本操作步骤 中的示例介绍了如何将 Nexus 5596 交换机更换为 Nexus 3132Q-V 交换机。您可以使用这些步骤（以及修改）来更换其他较早的 Cisco 交换机。操作步骤 使用以下交换机和节点命名：
...
* 要替换为 C2 （<<step20_replace5596_3232q,步骤 20>> - <<step33_replace5596_3232q,33>>）
+
...
* 已将两个 FAS9000 节点添加到集群中，其中的示例显示了集群详细信息（<<step34_replace5596_3232q,步骤 34>> - <<step37_replace5596_3232q,37>>）。


.步骤
. ` 步骤 1_replace5596_3232q]] 如果在此集群上启用了 AutoSupport ，请通过调用 AutoSupport 消息禁止自动创建案例： `ssystem node AutoSupport invoke -node * -type all -message MAINT=xh
+
...
----
+
如果要更换 Nexus 5010 或 5020 ，请从第 1 页为 ISL 指定适当的端口号。
...
----
. 【第 19 步 _replace5596_3232q]] 在所有节点上，还原连接到 C2 的所有已迁移集群互连 LIF ：
+
`网络接口还原`
...
----
. 【第 20 步 _replace5596_3232q]] 验证所有集群互连端口现在是否已还原到其主端口：
+
`network interface show`
...
----
. 【第 33 步 _replace5596_3232q]] 验证集群节点端口的状态： `network port show`
+
...
8 entries were displayed.
----
. 【第 34 步 _replace5596_3232q]] 在所有节点上，将特定集群 LIF 还原到其主端口：
+
`网络接口还原`
...
4 paths up, 0 paths down (udp check)
----
. 【第 37 步 _replace5596_3232q]] 通过向 Nexus 3132Q-V 集群交换机添加节点来扩展集群。
. 显示有关配置中的设备的信息：
+
** `network device-discovery show`
** `network port show -role cluster`
** `network interface show -role cluster`
** `scluster-switch show` + 以下示例显示了节点 n3 和 n4 ，其中 40 GbE 集群端口分别连接到两个 Nexus 3132Q-V 集群交换机上的端口 E1/7 和 E1/8 ，并且两个节点均已加入集群。使用的 40 GbE 集群互连端口为 E4A 和 e4e 。
+
[listing]
----
cluster::> network device-discovery show
...
----
. 如果未自动删除更换的 Nexus 5596 ，请将其删除： `ssystem cluster-switch delete`
+
