...
summary: '在 NVIDIA SN2100 集群交换机上安装或升级参考配置文件 (RCF) 脚本。' 
---
= 在 NVIDIA SN2100 集群交换机上安装或升级 RCF 脚本
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 NVIDIA SN2100 集群交换机上安装或升级参考配置文件 (RCF) 脚本，以配置端口分配和网络设置。

.开始之前
在安装或升级 RCF 脚本之前，请确保交换机上具备以下条件：

* Cumulus Linux 已安装。有关受支持的版本，请参见link:https://hwu.netapp.com/Switch/Index["Hardware Universe"^]。
...
| 集群 | 在配置为 4x10GbE 分支的端口上，一个端口配置为 4x25GbE 分支，其他端口配置为 40/100GbE。支持使用共享集群/HA 端口的节点端口上的共享集群/HA 流量。请参阅知识库文章中的平台表 link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports["哪些AFF、 ASA和FAS平台使用共享集群和 HA 以太网端口？"^]。所有端口也可用作专用集群端口。 
...
--
. 将集群交换机连接到管理网络。
. 使用 `ping`用于验证与托管 Cumulus Linux 和 RCF 的服务器的连接性的命令。
...
NOTE: 尽管 `scp`如果示例中使用的是这种方式，您可以使用您喜欢的文件传输方式，例如 SFTP、HTTPS 或 FTP。
...
. 重新应用之前对交换机配置所做的所有自定义设置。有关所需的任何进一步更改的详细信息，请参阅 link:cabling-considerations-sn2100-cluster.html["查看布线和配置指南"]。
. 重启后验证配置：
+
[listing, subs="+quotes"]
----
...
NOTE: 在上述步骤 3“更新 MOTD 文件”中，该命令 `cat /etc/issue.net`正在运行。这样，您就可以验证 RCF 文件名、RCF 版本、要使用的端口以及 RCF 横幅中的其他重要信息。
...
NOTE: 虽然 `scp`如果示例中使用的是这种方式，您可以使用您喜欢的文件传输方式，例如 SFTP、HTTPS 或 FTP。
...
NOTE: 在上述步骤 3 *更新 MOTD 文件* 中，运行命令 `cat /etc/issue.net`。这样，您就可以验证 RCF 文件名、RCF 版本、要使用的端口以及 RCF 横幅中的其他重要信息。
...
====
.下一步是什么？
安装完 RCF 后，您可以…… link:setup-install-cshm-file.html["安装 CSHM 文件"]。
