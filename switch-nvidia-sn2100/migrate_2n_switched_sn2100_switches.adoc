...
[role="lead"]
在无中断地将双节点无交换机集群迁移到使用NVIDIA SN2100集群交换机的集群时、您必须了解某些配置信息、端口连接和布线要求。您使用的操作步骤取决于每个控制器上是有两个专用集群网络端口，还是每个控制器上有一个集群端口。记录的过程适用于使用光纤或双轴端口的所有节点、但如果节点将板载10GBASE-T RJ45端口用于集群网络端口、则此交换机不支持此过程。

.双节点无交换机配置
* 双节点无交换机配置必须已正确设置并正常运行。
* 节点必须运行ONTAP 9.10.1P3及更高版本。
* 所有集群端口都必须处于已启动状态。
...
* 两台交换机都必须具有管理网络连接。
* 必须能够通过控制台访问集群交换机。
* NVIDIA SN2100节点到节点交换机和交换机到交换机连接必须使用双轴电缆或光缆。
+

IMPORTANT: 请参见 link:install_cabling_config_considerations_sn2100.html["布线和配置注意事项"^] 了解注意事项和更多详细信息。

+
。 https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe —交换机"^] 包含有关布线的详细信息。

* 交换机间链路(ISL)缆线必须连接到两个NVIDIA SN2100交换机上的端口swp15和swp16。
* 必须完成两个SN2100交换机的初始自定义。这样：
+
...
----
. 确认端口swp15和swp16上的ISL以及两个SN2100交换机sw1和SW2之间的ISL上的物理端口均已启动：`net show interface`
+
...
----
. 验证所有集群端口是否均已启动：`network port show`
+
...
----
. 确认所有集群LIF均已启动且正常运行：`network interface show`
+
...
cumulus@sw1:~$ *net commit*
----
. 在交换机sw1上、验证所有端口是否均已启动：`net show interface all`
+
[listing, subs="+quotes"]
----
...
cumulus@sw2:~$ *net commit*
----
. 在交换机SW2上、验证所有端口是否均已启动：`net show interface all`
+
[listing, subs="+quotes"]
----
...
...
----
. 在两个交换机sw1和SW2上、验证两个节点与每个交换机都有一个连接：`net show lldp`
+
...
----
. 显示有关集群中发现的网络设备的信息：`net device-discovery show -protocol lldp`
+
[listing, subs="+quotes"]
----
...
----
. 在所有集群LIF上启用自动还原：`net interface modify -vserver cluster -lif *-auto-revert true`
+
[listing, subs="+quotes"]
----
...
Cluster
          ...
          node2_clus2   true
----
. 确认`的所有接口均显示true、并且为Home`：`net interface show -vserver Cluster`
+
...
----
. 验证这些设置是否已禁用： `network options switchless-cluster show`
+
