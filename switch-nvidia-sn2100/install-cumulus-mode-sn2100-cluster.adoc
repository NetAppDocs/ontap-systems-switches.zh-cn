...
summary: 当交换机在 Cumulus 模式下运行时，在 NVIDIA SN2100 集群交换机上安装 Cumulus Linux。 
---
= 在 NVIDIA SN2100 集群交换机上以 Cumulus 模式安装 Cumulus Linux
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当 NVIDIA SN2100 集群交换机在 Cumulus 模式下运行时，在其上安装 Cumulus Linux (CL)。


NOTE: 可以在交换机运行 Cumulus Linux 或 ONIE 时安装 Cumulus Linux (CL) 操作系统（请参阅link:install-onie-mode-sn2100-cluster.html["以 ONIE 模式安装"]）。
...
----
+
有关详细信息，请参见 https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-54/System-Configuration/Authentication-Authorization-and-Accounting/User-Accounts/["NVIDIA用户帐户"^]。
