...
summary: 当交换机在 ONIE 模式下运行时，在 NVIDIA SN2100 集群交换机上安装 Cumulus Linux。 
---
= 在 NVIDIA SN2100 集群交换机上以 ONIE 模式安装 Cumulus Linux
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当 NVIDIA SN2100 集群交换机在 ONIE 模式下运行时，在其上安装 Cumulus Linux (CL)。


NOTE: 可以在交换机运行 ONIE 或 Cumulus Linux 时安装 Cumulus Linux (CL) 操作系统（请参阅link:install-cumulus-mode-sn2100-cluster.html["以 Cumulus 模式安装"]）。
