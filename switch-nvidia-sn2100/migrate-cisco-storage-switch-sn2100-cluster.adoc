...
summary: 从旧的 Cisco 存储交换机迁移到 NVIDIA SN2100 存储交换机，而不会中断存储网络操作。 
---
= 从 Cisco 存储交换机迁移到 NVIDIA SN2100 存储交换机
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
从旧的 Cisco 存储交换机迁移到 NVIDIA SN2100 存储交换机，而不会中断存储网络操作。
...
在使用 NVIDIA SN2100 存储交换机替换某些较旧的 Cisco 交换机时，您必须了解某些配置信息、端口连接和布线要求。请参阅 link:configure-overview-sn2100-cluster.html["NVIDIA SN2100交换机的安装和配置概述"]。
...
* NVIDIA SN2100 交换机上的某些端口配置为以 100 GbE 运行。
...
----
====
. 从节点的角度来看，每个节点上的存储端口通过以下命令连接到现有的存储交换机：
+
...
----
====
. 在交换机 S1 和 S2 上，存储端口和交换机通过以下命令连接（从交换机的角度来看）：
+
`show lldp neighbors`
...
----
====
. 从节点的角度来看，每个节点上的存储端口现在按以下方式连接到交换机：
+
.显示示例
[%collapsible]
====
