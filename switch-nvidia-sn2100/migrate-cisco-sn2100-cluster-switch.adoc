...
[role="lead"]
从 Cisco 集群交换机迁移到 NVIDIA SN2100 集群交换机，而不会中断集群操作。
...
在使用 NVIDIA SN2100 集群交换机替换某些较旧的 Cisco 集群交换机时，您必须了解某些配置信息、端口连接和布线要求。请参阅 link:configure-overview-sn2100-cluster.html["NVIDIA SN2100交换机的安装和配置概述"]。
...
有关支持的端口及其配置的详细信息，请参阅 link:https://hwu.netapp.com/["Hardware Universe"^]。
...
* NVIDIA SN2100 集群交换机已配置并运行在安装了正确版本的 Cumulus Linux 下，并应用了参考配置文件 (RCF)。
* 现有集群网络配置如下：
+
...
* NVIDIA SN2100 交换机上的一些端口配置为以 40 GbE 或 100 GbE 运行。
* 您已规划、迁移并记录了从节点到NVIDIA SN2100 集群交换机的 40 GbE 和 100 GbE 连接。



NOTE: 如果要更改 AFF A800 或 AFF C800 系统上 e0a 和 e1a 集群端口的端口速度，您可能会在速度转换后看到收到格式错误的数据包。有关指导，请参阅 link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1570339["漏洞编号 1570339"^] 和知识库文章 link:https://kb.netapp.com/onprem/ontap/hardware/CRC_errors_on_T6_ports_after_converting_from_40GbE_to_100GbE["将 40GbE 转换为 100GbE 后，T6 端口出现 CRC 错误"^]。
...
.关于示例
在此过程中，使用Cisco Nexus 3232C 集群交换机作为示例命令和输出。
...
+
每个 LIF 都应显示 `up/up`为了 `Status Admin/Oper`确实如此 `Is Home`。
...
. 从节点的角度来看，每个节点上的集群端口与现有集群交换机的连接方式如下：
+
