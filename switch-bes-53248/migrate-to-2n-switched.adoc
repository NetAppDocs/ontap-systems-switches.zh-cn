...
+
。 https://["_NetApp Hardware Universe"^] 包含有关ONTAP 兼容性、支持的EFOS固件以及连接到BES-53248交换机的布线的信息。
...
+
。 https://["_NetApp Hardware Universe"^] 包含有关平台的实际集群端口的最新信息。
...
+
。 https://["_NetApp Hardware Universe"^] 包含有关布线的详细信息。
...
====
+
有关详细信息，请参见 https://["NetApp 知识库文章： How to suppress automatic case creation during scheduled maintenance windows."^]
