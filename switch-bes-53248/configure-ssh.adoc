...
summary: '在 Broadcom BES-53248 集群交换机上启用 SSH，以支持以太网交换机健康监视器 (CSHM) 和日志收集功能。' 
---
= 在 Broadcom BES-53248 集群交换机上启用 SSH
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 Broadcom BES-53248 集群交换机上启用 SSH，以支持以太网交换机运行状况监控器 (CSHM) 和日志收集功能。此过程包括生成 SSH 密钥和激活交换机上的 SSH 访问。
