...
[role="lead"]
配置 BES-53248 集群交换机并应用新许可证后，即可安装参考配置文件 (RCF)。
...
CAUTION: 在安装新的交换机软件版本和 RCF 之前，请先阅读知识库文章。 https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["如何在保持远程连接的情况下清除博通互连交换机上的配置"^] 。如果必须彻底清除交换机设置，则需要重新进行基本配置。您必须使用串行控制台连接到交换机，因为完全清除配置会重置管理网络的配置。
...
. 请注意以下事项：
+
.. 如果交换机上安装了额外的端口许可证，则必须修改 RCF 以配置额外的许可端口。看link:configure-licenses.html#activate-newly-licensed-ports["激活新授权的端口"]了解详情。
