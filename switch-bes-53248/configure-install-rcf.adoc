...
如果要从旧版本升级 RCF ，则必须重置 Broadcom 交换机设置并执行基本配置以重新应用 RCF 。每次要升级或更改 RCF 时，都必须执行此操作。请参见 https://["知识库文章"^] 了解详细信息。
...
* 可从获取的当前RCF文件 https://["Broadcom 集群交换机"^] 页面。
* RC框架 中反映所需启动映像的启动配置、如果您仅安装EFOS并保留当前RC框架 版本、则需要此配置。如果您需要更改启动配置以反映当前启动映像，则必须在重新应用 RCF 之前进行更改，以便在将来重新启动时实例化正确的版本。
* 从出厂默认状态安装RC框架 时需要与交换机建立控制台连接。如果您已使用知识库文章、则此要求是可选的  https://["如何在保持远程连接的同时清除Broadcom互连交换机上的配置"^] 要清除配置、请事先清除。


.建议的文档
* 有关支持的ONTAP 和RCF版本、请参见交换机兼容性表。请参见 https://["EFOS软件下载"^] 页面。请注意、RC框架 中的命令语法与EFOS版本中的命令语法之间可能存在命令依赖关系。
* 请参阅上提供的相应软件和升级指南 https://["Broadcom"^] 有关BES-53248交换机升级和降级过程的完整文档的站点。
...
* `cluster1 ：：： * >` 提示符指示集群的名称。
* 此操作步骤 中的示例使用四个节点。这些节点使用两个10GbE集群互连端口 `e0a` 和 `e0b`。请参见 https://["Hardware Universe"^] 验证平台上的集群端口是否正确。
...
CAUTION: 在安装新交换机软件版本和RCF之前、请使用 https://["知识库文章：如何在保持远程连接的同时清除Broadcom互连交换机上的配置"^]。如果您必须完全擦除交换机设置、则需要重新执行基本配置。您必须使用串行控制台连接到交换机、因为完整的配置清除会重置管理网络的配置。
...
. 检查每个集群端口的管理和运行状态。
+
.. 验证所有集群端口是否均已启动且状态正常： `network port show –role cluster`
+
.显示示例
[%collapsible]
====
...
----
====
.. 验证所有集群接口（ LIF ）是否均位于主端口上： `network interface show -role cluster`
+
.显示示例
[%collapsible]
====
...
----
====
. 如果您尚未保存当前交换机配置、请将以下命令的输出复制到日志文件中以保存此配置： `show running-config`
...
----
====
. 检查中的横幅输出 `show clibanner` 命令：您必须阅读并遵循这些说明，以确保交换机的配置和操作正确。
+
.显示示例
[%collapsible]
====
...
----
====
. 验证交换机CS2上的端口： `show interfaces status all | exclude Detach`
+
.显示示例
[%collapsible]
====
...
. 验证集群上集群端口的运行状况。
+
.. 验证集群中所有节点上的e0b端口是否均已启动且运行正常： `network port show -role cluster`
+
.显示示例
[%collapsible]
====
...
--
====
. [[Step17]]在集群交换机CS1上、关闭连接到节点集群端口的端口。
+
以下示例使用接口示例输出：
...
. 重新启动交换机 CS1 。执行此操作可触发集群 LIF 还原到其主端口。您可以忽略在交换机重新启动时报告的节点上的 " 集群端口关闭 " 事件。
+
.显示示例
[%collapsible]
====
...
----
====
. 验证交换机CS1和CS2之间的ISL是否正常运行： `show port-channel 1/1`
+
.显示示例
[%collapsible]
====
