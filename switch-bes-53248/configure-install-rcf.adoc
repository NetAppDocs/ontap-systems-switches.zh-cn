...
summary: '完成初始配置并应用所需许可证后，在 Broadcom BES-53248 集群交换机上安装参考配置文件 (RCF)。' 
---
= 在 Broadcom BES-53248 集群交换机上安装 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
完成初始配置并应用所需许可证后，在 Broadcom BES-53248 集群交换机上安装参考配置文件 (RCF)。RCF 定义集群操作的端口分配、VLAN 和网络设置。


CAUTION: 对于 EFOS 3.12 及更高版本，请按照 link:efos-install-rcf-license-file.html["安装参考配置文件 (RCF) 和许可证文件"] 中的安装步骤进行操作。
...
* 当前 RCF 文件，可从 link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["博通集群交换机"^] 页面获得。
* RCF 中的启动配置反映了所需的启动映像，如果您仅安装 EFOS 并保留当前的 RCF 版本，则需要此配置。如果需要更改启动配置以反映当前的启动映像，则必须在重新应用 RCF 之前进行更改，以便在以后的重启中实例化正确的版本。
* 从出厂默认状态安装 RCF 时需要与交换机的控制台连接。如果您事先已使用知识库文章 link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["如何在保持远程连接的情况下清除博通互连交换机上的配置"^] 清除配置，则此要求是可选的。


.建议的文档
有关支持的 ONTAP 和 RCF 版本，请参阅交换机兼容性表。请参阅 link:https://mysupport.netapp.com/site/info/broadcom-cluster-switch["EFOS软件下载"^]。请注意，RCF 中的命令语法与 EFOS 版本中的命令语法之间可能存在命令依赖关系。
...
* 此过程中的示例使用四个节点。这些节点使用两个 10GbE 集群互连端口 `e0a` 和 `e0b`。请参阅link:https://hwu.netapp.com/Home/Index["Hardware Universe"^]以验证平台上正确的集群端口。
...
CAUTION: 在安装新的交换机软件版本和 RCF 之前，请参阅知识库文章 link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["如何在保持远程连接的情况下清除博通互连交换机上的配置"^]。如果必须完全擦除交换机设置，则需要重新执行基本配置。您必须使用串行控制台连接到交换机，因为完整的配置擦除会重置管理网络的配置。
...
show isdp neighbor
----
. 在集群交换机 cs2 上，关闭连接到节点集群端口的端口。例如，如果端口 0/1 到 0/16 连接到ONTAP节点：
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
. 请注意以下事项：
+
.. 如果已在交换机上安装了其他端口许可证，则必须修改 RCF 以配置其他许可证端口。有关详细信息，请参阅 link:configure-licenses.html#activate-newly-licensed-ports["激活新授权的端口"]。
...
--
====
. 检查横幅输出 `show clibanner`命令。您必须阅读并遵循这些说明，以验证交换机的配置和运行是否正确。
+
.显示示例
[%collapsible]
====
...
----
. 在集群交换机 cs2 上，启动连接到节点集群端口的端口。例如，如果端口 0/1 到 0/16 连接到ONTAP节点：
+
[listing, subs="+quotes"]
----
(cs2)> *enable*
...
--
====
. [[step20]]在集群交换机 cs1 上，关闭与节点集群端口连接的端口。
+
以下示例使用接口示例输出：
...
----
. 重启交换机cs1。这将触发集群 LIF 恢复到其原有端口。交换机重启期间，您可以忽略节点上报告的“集群端口关闭”事件。
+
[listing, subs="+quotes"]
----
(cs1)# *reload*
...
.下一步是什么？
安装完 RCF 后，您可以…… link:configure-ssh.html["启用 SSH"]。
