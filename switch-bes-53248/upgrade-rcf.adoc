...
summary: '升级 EFOS 软件后，升级 Broadcom BES-53248 集群交换机上的参考配置文件 (RCF)。' 
---
= 升级 Broadcom BES-53248 集群交换机上的 RCF
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在升级 EFOS 软件并应用任何所需的许可证后，升级 Broadcom BES-53248 集群交换机上的参考配置文件 (RCF)。
...
您可以使用交换机 CLI 和 ONTAP CLI（如以下步骤所示）进行端口配置。您可以在第一台交换机（cs2）上配置端口，然后对第二台交换机（cs1）重复这些步骤。
...
CAUTION: 更新或应用新的 RCF 时，您必须擦除交换机设置并执行基本配置。您必须使用串行控制台连接到交换机才能擦除交换机设置。
...
.下一步是什么？
升级 RCF 后，您可以 link:configure-ssh.html["为日志收集启用 SSH"]。
