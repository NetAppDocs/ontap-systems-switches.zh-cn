...
summary: 您可以在配置 BES-53248 集群交换机和应用任何新许可证后升级 RCF。 
---
= 升级 RCF 版本
:allow-uri-read: 
:icons: font
...
[role="lead"]
您可以在升级 BES-53248 集群交换机 EFOS 和应用任何新许可证后升级 RCF。
...
* 有关 BES-53248 交换机升级和降级过程的完整文档，请参见 link:https://www.broadcom.com/support/bes-switch["博通"^] 站点上提供的相应软件和升级指南。
...
. 请注意以下事项：
+
.. 如果交换机上已安装其他端口许可证，则必须修改 RCF 以配置其他许可证端口。有关详细信息，请参阅 link:configure-licenses.html#activate-newly-licensed-ports["激活新授权的端口"]。但是，当您升级到 RCF 1.12 或更高版本时，不再需要进行修改，因为所有接口现在都已预配置。
...
CAUTION: 上传并应用 RCF 文件时，必须将文件命名为 `reference-config.scr`，否则脚本应用步骤将失败。

. 使用以下传输协议之一将 RCF 复制到交换机 cs2：HTTP、HTTPS、FTP、SFTP 或 SCP。
+
...
----
+
有关更多详细信息，请参阅 link:https://docs.netapp.com/us-en/ontap/networking/revert_a_lif_to_its_home_port.html["将 LIF 恢复到其母端口"]。
