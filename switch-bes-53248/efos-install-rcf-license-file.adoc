...
summary: 完成初始配置后，在 Broadcom BES-53248 集群交换机上安装参考配置文件（RCF）和许可证文件。 
---
= 在 Broadcom BES-53248 集群交换机上安装 RCF 和许可证文件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 Broadcom BES-53248 集群交换机上同时安装参考配置文件 (RCF) 和许可证文件。从 EFOS 3.12.0.1 开始，您可以在完成初始交换机配置后安装这两个文件。


CAUTION: 安装 RCF 时所有端口均已配置，但您需要安装许可证才能激活已配置的端口。
...
* 当前 RCF，可从 link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["博通集群交换机"^] 页面获得。
...
. 使用以下传输协议之一将 RCF 复制到交换机 cs2 的 bootflash：FTP、HTTP、TFTP、SFTP 或 SCP。
+
...
--
====
. [[step21]]在集群交换机 cs1 上，关闭连接到节点集群端口的端口。
+
以下示例使用接口示例输出：
...
.下一步是什么？
安装完 RCF 和许可证文件后，您可以…… link:configure-ssh.html["启用 SSH"]。
