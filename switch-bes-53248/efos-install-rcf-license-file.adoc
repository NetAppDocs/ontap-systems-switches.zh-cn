...
---
= 安装参考配置文件(Reference Configuration File、RCF)和许可证文件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
从EFOS 3.12.0.1开始、您可以在配置BES-53248集群交换机后安装参考配置文件(Reference Configuration File、RCF)和许可证文件。


CAUTION: 安装RC框架 时会配置所有端口、但您需要安装许可证才能激活已配置的端口。
...
* 页面中提供的当前RC框架 https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Broadcom 集群交换机"^]。
...
. 使用以下传输协议之一将RCC复制到交换机CS2的bootflash：FTP、HTTP、TFTP、SFTP或SCP。
+
此示例显示了使用 HTTP 将 RCF 复制到交换机 CS2 上的 bootflash 。
...
--
====
. [[Step21]]在集群交换机CS1上、关闭连接到节点集群端口的端口。
+
以下示例使用接口示例输出：
