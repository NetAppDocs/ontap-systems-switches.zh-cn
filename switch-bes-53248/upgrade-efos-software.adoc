...
summary: '升级 Broadcom BES-53248 集群交换机上的以太网架构操作系统 (EFOS) 软件，以保持与 ONTAP 的兼容性。' 
---
= 升级 Broadcom BES-53248 集群交换机上的 EFOS 软件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
升级 Broadcom BES-53248 集群交换机上的以太网结构操作系统 (EFOS) 软件，以保持与 ONTAP 的兼容性并访问最新的交换机功能和修复。
...
.开始之前
* 从 link:https://www.broadcom.com/support/bes-switch["博通以太网交换机支持"^] 站点下载适用于您的集群交换机的 Broadcom EFOS 软件。
* 请查看以下有关 EFOS 版本的说明。
...
IMPORTANT: 请注意，将 BES-53248 集群交换机从 EFOS 3.3.x.x 或 3.4.x.x 升级到 EFOS 3.7.0.4 或 3.8.0.2 后，交换机间链路 (ISL) 和端口通道将标记为 *Down* 状态。这是预期的行为，除非您在自动恢复 LIF 时遇到问题，否则继续升级是安全的。有关更多详细信息，请参阅知识库文章 link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Systems/Fabric%2C_Interconnect_and_Management_Switches/BES-53248_Cluster_Switch_NDU_failed_upgrade_to_EFOS_3.7.0.4_and_later["BES-53248 集群交换机 NDU 升级到 EFOS 3.7.0.4 及更高版本失败"^]。
...
NOTE: 如果您此时也在升级 RCF，请按照以下步骤先升级交换机 cs2 上的 RCF，然后再升级交换机 cs1 上的 EFOS。有关详细信息，请参阅 link:upgrade-rcf.html["升级 RCF 版本"]。

. [[step12]] 在交换机 cs1 上重复步骤 5 至 11。
. 启用集群 LIF 的自动回滚功能。
+
[source, cli]
----
