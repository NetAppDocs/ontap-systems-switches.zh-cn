...
* BES-53248交换机上的某些端口配置为以10GbE或40GbE速度运行。
* 从节点到BES-53248集群交换机的10GbE和40GbE连接已进行规划、迁移和记录。
...
----
====
. 验证集群LIFs是否已迁移到集群交换机CL1上托管的端口。这可能需要几秒钟的时间。
+
`network interface show -vserver cluster`
...
.ONTAP 9.4及更早版本
--
对于ONTAP 9.4及更早版本、请使用命令： `system cluster-switch delete -device _device-name_`
