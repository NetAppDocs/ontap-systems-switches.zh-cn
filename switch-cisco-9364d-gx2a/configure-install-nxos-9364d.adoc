...
summary: 在 Cisco Nexus 9364D-GX2A 交换机上安装 NX-OS，用于新的 AFX 1K 或 AFX 2K 系统安装、交换机更换或出厂默认重置。 
---
= 在 Cisco Nexus 9364D-GX2A 交换机上安装 NX-OS
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 Cisco Nexus 9364D-GX2A 交换机上安装 NX-OS，用于新的 AFX 1K 或 AFX 2K 系统安装、交换机更换或将交换机重置为出厂默认设置。
...
+
** 如果您的交换机具有当前 NetApp 支持的版本，则无需安装 NX-OS。您需要安装 RCF。从 link:configure-install-upgrade-rcf-overview-9364d.html["安装或升级 RCF 概述"] 开始。
...
.下一步是什么？
在交换机上安装 NX-OS 后，从 link:configure-install-upgrade-rcf-overview-9364d.html["安装或升级 RCF 概述"] 开始安装 RCF。
