...
summary: 升级 Cisco Nexus 9364D-GX2A 交换机上的 NX-OS，以支持 AFX 1K 或 AFX 2K 系统上的新硬件或新 ONTAP 版本。 
---
= 在 Cisco Nexus 9364D-GX2A 交换机上升级 NX-OS
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
升级 Cisco Nexus 9364D-GX2A 交换机上的 NX-OS，以支持 AFX 1K 或 AFX 2K 系统上的新硬件或新 ONTAP 版本。
...
您在第一台交换机（cs1）上安装并验证 NX-OS 和 EPLD 映像，验证集群上所有端口的运行状况，然后在第二台交换机（cs2）上重复这些步骤。

. 安装 NX-OS 和 EPLD 映像。这些步骤可能因您当前的 NX-OS 版本和目标 NX-OS 版本而异。
+
选择适用于您情况的选项卡：

+
** 从10.4(2)F升级到10.6(3)F或更高版本
** 从10.6(3)F升级到更高版本
...
.从10.4(2)F升级到10.6(3)F或更高版本
--
您可以使用交换机 CLI 输入以下步骤的命令。

. [[stepa]] 将 NX-OS 和 EPLD 映像复制到第一台交换机。
+
...
[NOTE]
====
升级到 NX-OS 10.6(3)F 及更高版本时，EPLD 映像不会自动更新。升级完成并重新启动交换机后，必须再次运行相同的升级 `install all`命令。

** 第二次升级通过验证，但跳过 NX-OS 升级；仅更新 EPLD 映像。
** 当您已经升级到 NX-OS 10.6(3)F 及更高版本时，两个映像将同时更新。


====
+
安装映像文件会导致每次交换机重新启动时都会加载该映像文件。
