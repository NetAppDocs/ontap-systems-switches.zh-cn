...
summary: 确认已在 Cisco Nexus 9336C-FX2 存储交换机上启用 SSH 和 SSH 密钥，以支持 CSHM 和日志收集。 
---
= 验证 Cisco Nexus 9336C-FX2 存储交换机的 SSH 配置
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
验证是否在 Cisco Nexus 9336C-FX2 和 9336C-FX2-T 存储交换机上启用了 SSH 和 SSH 密钥，以支持以太网交换机健康监视器 (CSHM) 和日志收集功能。
...
.下一步是什么？
验证完 SSH 配置后，您link:../switch-cshm/config-overview.html["配置交换机健康监控"]。
